...
// How to use:
// ./releases2xml.js < /path/to/CHANGES.md
    ...
    return new Promise((resolve, reject) => {
        rl.on("line", line => {
            ...
            if(/\d\.\d.*\d{4}\s*$/.test(line)) {
                let param = match(line, /(\d\.\d(\.\d(\.\d)?)?)\W*(.*)$/);
                release = {
                    version: param(1),
            ...
            // release item
                ...
                let param = match(line, /^\s*\*\s+(.*)$/);
                release.items.push(param(1).trim());
            }
        });
...
function printXML(releases)
{
    ...
    const p1 = print(2), p2 = print(4), p3 = print(6), p4 = print(8), p5 = print(10);
