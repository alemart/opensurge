...
player_t *player_create(const char *character_name)
{
    ...
    physicsactor_set_airdrag(p->pa, physicsactor_get_airdrag(p->pa) * (1.0f / c->multiplier.airdrag));
    ...
    if(player_get_collectibles() > 0 || player->shield_type != SH_NONE) {
        ...
        if(player->shield_type != SH_NONE) {
            ...
            for(int i = 0; i < r; i++) {
                ...
                surgescript_object_t* collectible = level_create_object("Scattered Collectible", player->actor->position);
                if(collectible != NULL) {
